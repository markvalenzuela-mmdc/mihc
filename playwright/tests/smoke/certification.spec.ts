...
import { certifications, certificationDescriptor } from '../../config/pages.config';
import {
  checkCta,
  checkKeyContent,
  checkPageLoads,
...
for (const slug of certifications) {
  test(`smoke: certification ${slug}`, async ({ page, context }, testInfo) => {
    const d = certificationDescriptor(slug);
